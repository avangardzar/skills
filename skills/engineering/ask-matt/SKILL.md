...
1. **`/grill-with-docs`** sharpens the idea by interview. Start here whenever you are **working in a working directory**: it's stateful, retaining what it learns in `CONTEXT.md` and ADRs. (No working directory? Use `/grill-me` instead, covered under Standalone. Both run the same `/grilling` primitive; `grill-with-docs` is the one that leaves a paper trail, which makes it the better of the two whenever a repo is there to leave it in.)
   ...
   **Rounds, or one question at a time?** Both leave the same paper trail, and the choice is about you, not the topic. `/grill-with-docs` asks in rounds: the whole frontier at once, answered in a batch. Take it when you know the topic well enough to answer straight off. `/grill-one-by-one` asks exactly one question, waits, records the answer with a `✅` line, and only then asks the next. Take it when the topic is new to you and you need to ask the agent questions back before you can answer: in rounds, that leaves a queue of unanswered questions piling up.
...
- **`/grilling`** is the interview primitive itself: rounds, the frontier, facts are the agent's job and decisions are yours. `/grill-me` and `/grill-with-docs` are the two named ways in (`/grill-one-by-one` runs the same interview with the rounds swapped for one question at a time, so it carries its own instructions rather than this primitive), and `/triage`, `/wayfinder` and `/improve-codebase-architecture` all run it internally. Reach for it directly only when you want the interview with no wrapper around it.
